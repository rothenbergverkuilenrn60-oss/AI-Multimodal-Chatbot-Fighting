...
class ErnieEngine:
                    ...
                    if item_type == "text":
                        ...
                        if file_path:
                            ...
                            if os.path.exists(normalized_path):
                                ...
                                content_parts.append({"type": "text", "text": f"[系统提示: 文件未找到 - {normalized_path}]"})
                                continue 
    ...
    def chat(self, history, model_name: str = "ernie-5.0-thinking-preview", temperature: float = 0.7, prompt_text: str = ""):
        # 1. 注入 System Prompt
        messages_payload = []
        ...
        if messages_payload and isinstance(messages_payload[-1].get("content"), list):
            ...
            has_media = any(item.get("type") == "image_url" or "视频文件" in str(item) for item in last_content)
        ...
        print(f"DEBUG: Payload sending to API (truncated): {debug_payload[:200]} ... [len: {len(debug_payload)}]")
