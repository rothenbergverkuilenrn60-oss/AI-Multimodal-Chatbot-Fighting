...
class QianfanLlmEngine:
    def __init__(self):
        ...
        if not api_key:
            raise ValueError("QIANFAN_API_KEY not set,or .env file not found.") 
    ...
    #--R08 增加角色提示词参数
    def chat(self,history,model_name:str="ernie-4.0-turbo-8k",temperature:float=0.7,prompt_text:str=""):
        # 1-预处理、清洗
        openai_messages=[]
        for msg in history:  
            ...
            if isinstance(raw_content,list):                
                ...
                clean_content = str(raw_content) if raw_content else ""  
        ...
        try:
            # 验证模型名称
            valid_models = ["ernie-4.0-turbo-8k", "ernie-3.5-8k", "ernie-speed-128k", "ernie-speed-8k"]
            if model_name not in valid_models:
                print(f"WARNING: Model {model_name} might not be supported. Trying default model...")
